...
const seedDatabase = async () => {
  try {
    ...
    console.log(`  -d '{"email":"admin@local.test","password":"${adminPassword}"}'`);
    console.log('  Response will include token in JSON and httpOnly cookie\n');

    console.log('2. Create service request (requires auth header):');
    console.log(`curl -X POST http://localhost:5000/api/requests \\`);
    console.log(`  -H "Content-Type: application/json" \\`);
    console.log(`  -H "Authorization: Bearer YOUR_TOKEN" \\`);
    console.log(`  -d '{"type":"Plumbing","details":"Leaking pipe","images":[]}'\\n`);

    console.log('3. Get all requests (admin only):');
    console.log(`curl -X GET http://localhost:5000/api/requests \\`);
    console.log(`  -H "Authorization: Bearer YOUR_TOKEN"\n`);
